...
def create_jwt_token(user_data: dict) -> str:
    ...
    return jwt.encode(payload, "auth-example-super-secret-key-change-in-production", algorithm="HS256")
...
def verify_jwt_token(token: str) -> Optional[dict]:
    ...
    try:
        if token not in active_tokens:
            return None
        payload = jwt.decode(token, "auth-example-super-secret-key-change-in-production", algorithms=["HS256"])
        return payload
    except JWTError:
        return None
