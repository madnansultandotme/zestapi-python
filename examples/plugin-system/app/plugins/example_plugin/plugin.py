...
class APIKeyMiddleware(BaseMiddleware):
    ...
    def __init__(self, api_keys: List[str], header_name: str = "X-API-Key", protected_paths: Optional[List[str]] = None):
        super().__init__()
...
class APIKeyPlugin(BasePlugin):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.protected_paths = self.config.get("protected_paths", ["/protected", "/admin"])
        self.middleware = None
