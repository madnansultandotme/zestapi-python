...
async def root(request):
    return ORJSONResponse({
        ...
        "version": "1.0.0",
        "plugins": [plugin.get_status() for plugin in app_plugins] if app_plugins else [],
        "endpoints": {
            "public": {
                "root": "GET /",
                "test": "GET /api/test",
...
async def plugin_status(request):
    ...
    return ORJSONResponse({
        ...
        "active_plugins": sum(1 for plugin in app_plugins if plugin.is_active) if app_plugins else 0
    })
...
async def startup():
    ...
    try:
        ...
        print("    curl -H 'X-API-Key: test-api-key-123' http://localhost:8000/protected")
        print("    curl -H 'X-API-Key: test-api-key-123' http://localhost:8000/admin")
        print("    curl -H 'X-API-Key: test-api-key-123' http://localhost:8000/api/secure")
