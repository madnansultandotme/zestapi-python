...
def check_git_status():
    ...
    env["HOME"] = "/c/Users/adnan"
    result = subprocess.run(
        "git status --porcelain", shell=True, capture_output=True, text=True, env=env
    )
...
def setup_git_config():
    ...
    if not run_command(
        'git config user.name "madnansultandotme"', "Setting Git username"
    ):
        return False

    if not run_command(
        'git config user.email "info.adnansultan@gmail.com"', "Setting Git email"
    ):
        return False
...
def deploy_to_github():
    ...
    if result.returncode != 0:
        commit_msg = input(
            "Enter commit message (or press Enter for 'Update project'): "
        )
        if not commit_msg:
...
def create_release_tag():
    ...
    result = subprocess.run(
        f"git tag -l v{version}", shell=True, capture_output=True, text=True
    )
    if result.stdout.strip():
